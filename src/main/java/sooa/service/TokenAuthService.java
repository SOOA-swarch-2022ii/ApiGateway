...
public class TokenAuthService {
    ...
    public User registerUserService(User usuario) {
        return rest.postForObject("http://35.241.129.8:6665/register-user",
                usuario, User.class);
    }

    public User getUserService(String username) {
        return rest.getForObject("http://35.241.129.8:6665/user/un={username}",
                User.class, username);
    }


    public TokenLogin loginUserService ( LoginForm loginForm) {
        return rest.postForObject("http://35.241.129.8:6665/login",
